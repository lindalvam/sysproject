<?php 
class projeto_fase extends super{
    var $tabela='projeto_fase';
	var $campos=array("id","id_projeto","fase", "ordem","created_at");
	var $campo_id='id';          
...
--------------------------------------------------------------*/
			...
			case "listar":
				...
				$this->procedimentoErro("Houve um erro na tentativa de listar as fases do projeto cadastrados. Consulte o log do sistema e contate o administrador.","alert","pop_area.php");
			break;
			...
			case "captaRegistro":
				...
				if($this->numRegs<1){
					die('<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
                         <!--
                         alert("Nenhuma fase do projeto foi encontrado.");						 
                         //-->
                         </SCRIPT>');
				}
			break;
...
--------------------------------------------------------------*/
			case "salvar":
				...
				if($this->erros!=array()){ // Houve erros ao salvar o registro
					die(utf8_decode('<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
	                     <!--
						 alert("Houve um erro na tentativa de gravar a fase do projeto. Consulte o log do sistema.");
	                     history.go(-1);
	                     //-->
	                     </SCRIPT>'));
				}else{
						$acao=$args[$this->campo_id]==""?"incluído":"alterado";
						$d="Fase do projeto $acao com sucesso!";
						die('<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
						<!--
			...
			case "excluir":
				...
				if($this->erros!=array()){ // Houve erros na gravação de dados
                    $d="Houve um erro na tentativa de excluir a fase do projeto. Consulte o log do sistema e contate o administrador.";
